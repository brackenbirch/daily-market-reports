const createMarketPrompt = (marketData) => `You are a senior financial analyst creating a daily market summary with EXACT real-time data for institutional clients.

${formatMarketDataForPrompt(marketData)}

CRITICAL: Use the EXACT numbers provided above. These are real-time market prices to the penny and you must reference them precisely.

Create a professional report with these exact sections using the real-time data:

**EXECUTIVE SUMMARY**
Use the exact SPY, QQQ, DIA prices and changes shown above. Reference precise percentage moves.

**ASIAN MARKETS OVERNIGHT**
Create professional analysis and include exact currency rates:
- USD/JPY: ${marketData.currencies['USD/JPY'] || 'N/A'}
- USD/CNY: ${marketData.currencies['USD/CNY'] || 'N/A'}  
- AUD/USD: ${marketData.currencies['AUD/USD'] || 'N/A'}
Use realistic ranges for Asian indices.

**EUROPEAN MARKETS SUMMARY**  
Include exact currency rates:
- EUR/USD: ${marketData.currencies['EUR/USD'] || 'N/A'}
- GBP/USD: ${marketData.currencies['GBP/USD'] || 'N/A'}
Use realistic ranges for European indices.

**US MARKET OUTLOOK**
Reference the exact SPY, QQQ, DIA prices, changes, and volumes shown in the real-time data above.

**PREMARKET MOVERS**
List the exact top 10 gainers and losers with:
- Exact symbols, prices, and percentage changes as shown above
- Exact volume data where provided
- Note the real-time source attribution

**SECTOR ANALYSIS**
Use the exact SPDR ETF prices, changes, and volumes for each sector:
- **XLF (Financial Services)**: Use exact price ${marketData.sectors?.XLF?.price ? `${marketData.sectors.XLF.price}` : 'from data above'}
- **XLK (Technology)**: Use exact price ${marketData.sectors?.XLK?.price ? `${marketData.sectors.XLK.price}` : 'from data above'}
- **XLE (Energy)**: Use exact price ${marketData.sectors?.XLE?.price ? `${marketData.sectors.XLE.price}` : 'from data above'}
- **XLV (Healthcare)**: Use exact price ${marketData.sectors?.XLV?.price ? `${marketData.sectors.XLV.price}` : 'from data above'}
- **XLI (Industrials)**: Use exact price ${marketData.sectors?.XLI?.price ? `${marketData.sectors.XLI.price}` : 'from data above'}
- **XLY (Consumer Discretionary)**: Use exact price ${marketData.sectors?.XLY?.price ? `${marketData.sectors.XLY.price}` : 'from data above'}
- **XLP (Consumer Staples)**: Use exact price ${marketData.sectors?.XLP?.price ? `${marketData.sectors.XLP.price}` : 'from data above'}
- **XLU (Utilities)**: Use exact price ${marketData.sectors?.XLU?.price ? `${marketData.sectors.XLUconst axios = require('axios');
const fs = require('fs');
